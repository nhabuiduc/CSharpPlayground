app.controller('SyntaxTreeController',
    ['$scope', 'appService', 'csharpSyntaxService', '$interval',
        function ($scope, appService: AppService, csharpSyntaxService: CSharpSyntaxService, $interval: angular.IIntervalService) {
            $scope.context = {
                selectedNodes: [],
                syntaxTree: null,
            };
            ...
            var lastTree: Core.SyntaxTree;
            var lastPosition: number;
            var isNavigatingFromTreeToSource = false;
            var isCursorChanged = false;

            function refreshTree() {
                var tree = csharpSyntaxService.GetTree();
                if (lastTree != tree) {
                    populateTree(tree);
                    lastTree = tree;
                ...
                if (isCursorChanged) {
                    expandTreeToPosition(lastPosition);
                }
                isCursorChanged = false;
            }

            function expandTreeToPosition(position: number) {
            ...
            function populateTree(tree: Core.SyntaxTree) {
            ...
            (function () {
                var tree = csharpSyntaxService.ParseTree($('#sourceCode').val());
                populateDiagnostic(tree);
                var nodes = Linq.Enumerable.ToArray(tree.GetRoot().ChildNodesAndTokens());
